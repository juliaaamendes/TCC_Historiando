...
DELIMITER //
CREATE PROCEDURE RegistrarNovoJogador(
    IN nome VARCHAR(50),
    IN email VARCHAR(50),
    IN senha VARCHAR(8),
    IN progresso_id INT
...
BEGIN
    ...
    IF email_existente > 0 THEN
        SELECT 'Erro: Email já cadastrado' AS mensagem;
    ELSE
        INSERT INTO jogador (nome_jogador, email_jogador, senha_jogador, progresso_id)
        VALUES (nome, email, senha, progresso_id);
        SELECT 'Jogador cadastrado com sucesso!' AS mensagem;
    END IF;
END//
DELIMITER ;

DELIMITER //
CREATE PROCEDURE AtualizarProgressoJogador(
    IN jogador_id INT,
    IN nova_fase INT,
    IN novo_nivel INT,
    IN nova_conquista INT
)
BEGIN
    DECLARE progresso_atual INT;
    
    SELECT progresso_id INTO progresso_atual
    FROM jogador 
    WHERE id_jogador = jogador_id;
    
    UPDATE progresso 
    SET id_fase = nova_fase, 
        id_nivel = novo_nivel, 
        conquistaUsuario = nova_conquista
    WHERE id_progresso = progresso_atual;

    INSERT INTO historico (id_jogador, tempo)
    VALUES (jogador_id, NOW());
    
    SELECT 'Progresso atualizado com sucesso!' AS mensagem;
END//
DELIMITER ;

DELIMITER //
CREATE PROCEDURE ListarJogadoresPorConquista(IN nome_conquista VARCHAR(100))
BEGIN
    SELECT 
        j.nome_jogador,
        j.email_jogador,
        ...
        n.titulo_nivel
    FROM jogador j
    INNER JOIN progresso p ON j.progresso_id = p.id_progresso
    INNER JOIN conquista c ON p.conquistaUsuario = c.id_conquista
    INNER JOIN fase f ON p.id_fase = f.id_fase
    INNER JOIN nivel n ON p.id_nivel = n.id_nivel
    WHERE c.nome_conquista = nome_conquista;
END//
DELIMITER ;

DELIMITER //
CREATE PROCEDURE JogadoresAtivosPorPeriodo(
    IN data_inicio DATE,
    IN data_fim DATE
)
BEGIN
    SELECT 
        j.nome_jogador,
        j.email_jogador,
        COUNT(h.id_historico) AS 'sessoes',
        MIN(h.tempo) AS 'primeira_sessao',
        MAX(h.tempo) AS 'ultima_sessao'
    FROM jogador j
    INNER JOIN historico h ON j.id_jogador = h.id_jogador
    WHERE DATE(h.tempo) BETWEEN data_inicio AND data_fim
    GROUP BY j.id_jogador, j.nome_jogador, j.email_jogador
    ORDER BY sessoes DESC;
END//
DELIMITER ;

DELIMITER //
CREATE PROCEDURE RelatorioConquistasPorFase()
BEGIN
    SELECT 
        f.titulo_fase AS 'Fase',
        c.nome_conquista AS 'Conquista',
        COUNT(j.id_jogador) AS 'jogadores_com_conquista'
    FROM fase f
    INNER JOIN progresso p ON f.id_fase = p.id_fase
    INNER JOIN conquista c ON p.conquistaUsuario = c.id_conquista
    INNER JOIN jogador j ON p.id_progresso = j.progresso_id
    GROUP BY f.id_fase, f.titulo_fase, c.id_conquista, c.nome_conquista
    ORDER BY f.id_fase, jogadores_com_conquista DESC;
END//
DELIMITER ;

DELIMITER //
CREATE PROCEDURE LimparHistoricoAntigo(IN dias_manter INT)
BEGIN
    DECLARE registros_afetados INT;
    
    DELETE FROM historico 
    WHERE tempo < DATE_SUB(NOW(), INTERVAL dias_manter DAY);
    
    SET registros_afetados = ROW_COUNT();
    
    SELECT CONCAT('Registros removidos: ', registros_afetados) AS resultado;
END//
DELIMITER ;

--calls

CALL RegistrarNovoJogador('Novo Jogador', 'novo@email.com', 'senha123', 1);

CALL AtualizarProgressoJogador(1, 2, 4, 2);

CALL EstatisticasSistema();

CALL TopJogadoresPorProgresso(5);

CALL JogadoresAtivosPorPeriodo('2024-01-20', '2024-01-27');

CALL RelatorioConquistasPorFase();

CALL LimparHistoricoAntigo(30);